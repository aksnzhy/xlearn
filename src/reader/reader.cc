...
std::string Reader::check_file_format() {
    ...
    if (space_count > table_count && space_count > comma_count) {
        splitor_ = " ";
    } else if (table_count > space_count && table_count > comma_count) {
        splitor_ = "\t";
    } else if (comma_count > space_count && comma_count > table_count) {
        splitor_ = ",";
    } else {
...
void InmemReader::Initialize(const std::string& filename) {
    ...
    if (hash_binary(filename_)) {
        ...
        try {
            block_ = (char*)malloc(block_size_*1024*1024);
        } catch (std::bad_alloc&) {
