...
void FileSpliter::split(const std::string &filename, int num_blocks) {
    ...
    uint64 next_block_size = average_block_size + file_size - (average_block_size * num_blocks);
    uint64 offset = 0;
    // Output
    ...
    for (int i = 0; i < num_blocks; ++i) {
        ...
        int ret = ftruncate(file_desc_write[i], next_block_size+kMaxLineSize);
        CHECK_EQ(ret, 0);
    }
    char *map_ptr_read = (char *)mmap(NULL, file_size, PROT_READ, MAP_PRIVATE, file_desc_read, 0);
    CHECK_NE(map_ptr_read, MAP_FAILED);

    for (int i = 0; i < num_blocks; ++i) {
        char *map_ptr_write = (char *)mmap(NULL, next_block_size, PROT_WRITE, MAP_SHARED, file_desc_write[i], 0);
        ...
        CHECK_EQ(ret, 0);
        next_block_size = average_block_size + (next_block_size - real_file_size);
