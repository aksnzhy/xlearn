...
//------------------------------------------------------------------------------
// We use 32 bits float to store the real number, such as the
// model parameter and the gradient
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
// We use SSE to speedup our training and some hyper
...
struct Node {
  ...
  index_t feat_id;   /* feature id, Start from 0 */
...
// It can be the whole data set used in in-memory training, or just a
// working set in on-disk training, because for many large-scale ML
// problems, we cannot load all the training data into memory at once.
// So we can load a small batch of dataset in DMatrix at each samplling.
...
//    for (int i = 0; i < 10; ++i) {
//      matrix.Y[i] = ...
//      matrix.AddNode(i, feat_id, feat_val);
//    }
...
struct DMatrix {
  ...
  // between two data matrix. The hash value can be generated
  // by HashFile() method (file_util.h) and this value will be
  // used when reading txt data from disk file. We can cache the
  // binary data in disk file to speedup the reading
